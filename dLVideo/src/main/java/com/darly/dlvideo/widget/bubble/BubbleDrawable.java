package com.darly.dlvideo.widget.bubble;
...
/**
 * @author zhangyh2 BubbleDrawable 下午3:19:15 TODO 绘制功能，提供参数的一个Drawable对象
 */
public class BubbleDrawable extends Drawable {
	...
	private BubbleDrawable(Builder builder) {
		...
		this.pathBorder = builder.pathBorder;
	...
	private void setUp(Canvas canvas) {
		...
		case COLOR:
			...
			pathPaint.setStrokeWidth(pathBorder);
			break;
		case BITMAP:
	...
	private void setUpLeftPath(RectF rect, Path path) {
		...
		path.arcTo(new RectF(rect.right - mAngle, rect.top, rect.right, mAngle
				+ rect.top), 270, 90);
	...
	private void setUpTopPath(RectF rect, Path path) {
		path.moveTo(rect.left + Math.min(mArrowPosition, mAngle), rect.top
				+ mArrowHeight);
		path.lineTo(rect.left + mArrowPosition, rect.top + mArrowHeight);
		path.lineTo(rect.left + mArrowWidth / 2 + mArrowPosition, rect.top);
		path.lineTo(rect.left + mArrowWidth + mArrowPosition, rect.top
				+ mArrowHeight);
		path.lineTo(rect.right - mAngle, rect.top + mArrowHeight);

		path.arcTo(new RectF(rect.right - mAngle, rect.top + mArrowHeight,
				rect.right, mAngle + rect.top + mArrowHeight), 270, 90);
		path.lineTo(rect.right, rect.bottom - mAngle);
		...
		path.arcTo(new RectF(rect.left, rect.bottom - mAngle, mAngle
	...
	private void setUpRightPath(RectF rect, Path path) {
		...
		path.arcTo(new RectF(rect.right - mAngle - mArrowWidth, rect.top,
				rect.right - mArrowWidth, mAngle + rect.top), 270, 90);
		...
		path.arcTo(new RectF(rect.right - mAngle - mArrowWidth, rect.bottom
				- mAngle, rect.right - mArrowWidth, rect.bottom), 0, 90);
		path.lineTo(rect.left + mArrowWidth, rect.bottom);

		path.arcTo(new RectF(rect.left, rect.bottom - mAngle, mAngle
				+ rect.left, rect.bottom), 90, 90);

		path.arcTo(new RectF(rect.left, rect.top, mAngle + rect.left, mAngle
				+ rect.top), 180, 90);
		path.close();
	}
	...
	private void setUpBottomPath(RectF rect, Path path) {
		...
		path.arcTo(new RectF(rect.right - mAngle, rect.bottom - mAngle
				- mArrowHeight, rect.right, rect.bottom - mArrowHeight), 0, 90);

		path.lineTo(rect.left + mArrowWidth + mArrowPosition, rect.bottom
				- mArrowHeight);
		...
		path.lineTo(rect.left + Math.min(mAngle, mArrowPosition), rect.bottom
				- mArrowHeight);
	...
	public static class Builder {
		...
		public static float DEFAULT_PATH_BORDER = 2;
		private RectF mRect;
		...
		private float pathBorder = DEFAULT_PATH_BORDER;
		private int pathColor = DEFAULT_BUBBLE_COLOR;
		...
		public Builder pathBorder(float pathBorder) {
			this.pathBorder = pathBorder;
			return this;
		}

		public Builder pathColor(int pathColor) {
		...
		public BubbleDrawable build() {
			if (mRect == null) {
				throw new IllegalArgumentException(
						"BubbleDrawable Rect can not be null");
			}
