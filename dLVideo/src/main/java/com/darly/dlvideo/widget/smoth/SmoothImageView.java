 ...
 * TODO
 */
package com.darly.dlvideo.widget.smoth;
 ...
 * 
 * 使用的时候，从前一个Activity传递到详情Activity下面几个主要的信息： Intent intent = new
 * Intent(MainActivity.this, SpaceImageDetailActivity.class);
 * intent.putExtra("images", (ArrayList<String>) datas);//非必须
 * intent.putExtra("position", position); int[] location = new int[2];
 * imageView.getLocationOnScreen(location); intent.putExtra("locationX",
 * location[0]);//必须 intent.putExtra("locationY", location[1]);//必须
 * 
 * intent.putExtra("width", imageView.getWidth());//必须 intent.putExtra("height",
 * imageView.getHeight());//必须 startActivity(intent);
...
public class SmoothImageView extends ImageView {
	...
	public void setOriginalInfo(int width, int height, int locationX,
			int locationY) {
		...
		mOriginalLocationY = mOriginalLocationY
	...
	protected void getCenterCropMatrix() {
		if (getDrawable() == null) {
			return;
		}
		if (mBitmap == null || mBitmap.isRecycled()) {
		...
		mSmoothMatrix.postTranslate(
				-(scale * mBitmap.getWidth() / 2 - mOriginalWidth / 2), -(scale
						* mBitmap.getHeight() / 2 - mOriginalHeight / 2));
	}
	...
	protected void onDraw(Canvas canvas) {
		...
		if (mState == STATE_TRANSFORM_IN || mState == STATE_TRANSFORM_OUT) {
			...
			if (mTransformStart) {
				...
				Log.d("Dean",
						"mTransfrom.startRect:"
								+ mTransfrom.startRect.toString());
				Log.d("Dean",
						"mTransfrom.endRect:" + mTransfrom.endRect.toString());
	...
	private void startTransform(final int state) {
		...
		if (state == STATE_TRANSFORM_IN) {
			...
			PropertyValuesHolder widthHolder = PropertyValuesHolder.ofFloat(
					"width", mTransfrom.endRect.width,
					mTransfrom.startRect.width);
			PropertyValuesHolder heightHolder = PropertyValuesHolder.ofFloat(
					"height", mTransfrom.endRect.height,
		...
		valueAnimator
				.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
					...
					public synchronized void onAnimationUpdate(
						...
						mTransfrom.scale = (Float) animation
								.getAnimatedValue("scale");
						mTransfrom.rect.left = (Float) animation
								.getAnimatedValue("left");
						mTransfrom.rect.top = (Float) animation
								.getAnimatedValue("top");
						mTransfrom.rect.width = (Float) animation
								.getAnimatedValue("width");
						mTransfrom.rect.height = (Float) animation
								.getAnimatedValue("height");
						mBgAlpha = (Integer) animation
								.getAnimatedValue("alpha");
						invalidate();
