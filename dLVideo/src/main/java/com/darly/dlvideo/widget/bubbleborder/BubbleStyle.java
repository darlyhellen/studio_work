...
interface BubbleStyle {
	...
	enum ArrowDirection {
		...
		private static final Map<Integer, ArrowDirection> intToTypeMap = new HashMap<Integer, ArrowDirection>();
	...
	/**
	 * 设置箭头指向的View对象 设置了View对象后，setArrowPos将不起作用
	 *
	 * @param viewId
	 *            指向的ViewId
	 */
	 ...
	 *            左下角
	 */
	void setCornerRadius(float topLeft, float topRight, float bottomRight,
			float bottomLeft);
	...
	/**
	 * 设定Padding 将自动将箭头区域占用空间加入Padding，使内容能够完全被气泡包含
	 *
	 * @param left
	 *            用户指定的 LeftPadding
	 * @param top
