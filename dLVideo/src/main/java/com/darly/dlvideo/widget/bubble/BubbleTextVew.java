...
/**
 * @author zhangyh2 BubbleTextVew 下午3:19:48 TODO 带箭头的文本
 */
public class BubbleTextVew extends TextView {
	...
	private void initView(AttributeSet attrs) {
		if (attrs != null) {
			...
			pathBorder = array.getDimension(R.styleable.BubbleView_pathBorder,
					BubbleDrawable.Builder.DEFAULT_PATH_BORDER);
			pathColor = array.getColor(R.styleable.BubbleView_pathColor,
					BubbleDrawable.Builder.DEFAULT_BUBBLE_COLOR);
			array.recycle();
		}
		setUpPadding();
	...
	private void setUp(int left, int right, int top, int bottom) {
		...
		bubbleDrawable = new BubbleDrawable.Builder().rect(rectF)
				...
				.bubbleColor(bubbleColor).arrowPosition(mArrowPosition)
				.pathBorder(pathBorder).pathColor(pathColor).build();
	}
